...
import { brands, business, categories, products } from "@/lib/business";
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    <section className="shell px-5 py-10 sm:px-8 sm:py-16 lg:px-12"><SectionHeading eyebrow={labels.brand} title={labels.brandsTitle} text={labels.brandsText} /><div className="flex min-w-0 flex-wrap gap-2 sm:gap-3">{brands.map((brand) => <Link href={`/${locale}/shop?brand=${brand}`} className="max-w-full rounded-full border border-line bg-white px-4 py-3 font-display text-sm font-semibold text-stone-600 transition hover:border-gold hover:text-ink sm:px-5" key={brand}>{brand}</Link>)}</div></section>
